...
2. characters：抽取所有人物，每个人物给出：
   ⚠️【只收真人物，不收称谓、不把小名单列成人】：
     · "母亲/父亲/祖母/管家/掌柜"这类【身份称谓】不是人名，不要列成人物，
       也不要改——正文里叫"母亲"的地方就该还是"母亲"。
     · "小顾""沈二姑娘""渊儿"是【某个人的别称】，要放进那个人的 pairs 里，
       不要单独列成一个人物（那会在关系图里凭空多出一个人）。
   - original：原全名（如"沈砚之"）。
   ...
   - replacement：改名后的全名。规则：
     * 【保留姓】；姓之后【名字的每一个字都必须换掉】，换成读音相同或相近、但【字不同】
       ...
       苏文轩 → 苏雯萱（文→雯，轩→萱）。
     * 【全名严禁原样返回，配角也不例外】：任何角色（含配角）的 replacement 都
       【绝不能等于】original——姓可相同，名里【每一个字都要换成
   ...
   - pairs：该人物名下所有需要替换的 token → 新串。包含：全名、单独的名（去掉姓）、
     ...
     ⚠️【小名要找全】：正文里凡是用来叫这个人的都要收——
       渊儿、阿渊、小渊、渊哥、文渊哥哥、砚儿、砚少爷、晚丫头、阿晚…
       "名字+儿/哥哥/姐姐/丫头/少爷"这类【前半截是名字】的，整串都要收，
       换的时候只换名字那部分（砚少爷 → 彦少爷，"少爷"不动）。
     ⚠️【和大名不同源的乳名】：像大名"江砚辞"、乳名"阿宝"，"宝"和"砚辞"
       没有一个字相同。尽量也给它找一个同音字（阿宝 → 阿堡/阿葆）；
       但如果你【拿不准该换成什么】，就把 to 原样填成 from——
       这是允许的，会由人来填。**千万不要为了"看起来改过了"而瞎编**，
       也不要因为想不出来就【不列出这个小名】：漏列比不改严重得多，
       漏了的话人根本不知道有这个称呼要处理。
     ⚠️【最要紧的是准和对得上】：小名要挂在【正确的人】名下，不能张冠李戴；
       同一个人的所有称呼要收全。这两件事比"改得漂不漂亮"重要。
     同一人物的所有 token 必须映射到【一致】的新名：同一个原字在全名和小名里
     必须换成【同一个字】（渊→缘，那么 渊儿→缘儿、阿渊→阿缘，不能一个缘一个远）。每个 pair：
       { "from": 原串, "to": 新串,
         "global": 是否可全局替换,
         "contexts": 若 global=false 才给——原文里包含该 from、足以定位它的片段(数组) }
     * global=true：全名、或独特的多字别称（撞词概率低）。
     * global=false：单字名、叠字小名、或任何可能同时是普通词的 token。
       ⚠️【这条比"找全别名"更要紧】：叠字小名尤其危险——
         "微微"既是小名也是副词（她微微一笑）、"囡囡""朵朵""安安"同理；
         单字名会撞词（"砚"撞"砚台"、"晚"撞"晚上"、"晴"撞"晴天"）。
       这些【一律 global=false】并给 contexts，宁可漏改几处，也不能把
       "微微一笑"改成"薇薇一笑"——那是正文被改坏，比漏改严重得多。
