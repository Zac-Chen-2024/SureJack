...
async function buildFilm (
  ...
  if (masterReuse === null) {
     ...
     *
     * ⚠️【必须是烧录成功之后、就地删】。两点都要紧：
     *
     * · 提前删 → 烧录中途失败重试就得从重拼开始，而它本来可以直接复用。
     * · 不能挪到开机之类的地方【事后扫】。试过一版"开机回收母带已就绪的
     *   背景轨"，判据是"母带在 + 指纹说 done"——看着严谨，其实会误删：
     *   用户改了字幕触发重烧，配音就绪时会【预拼一条新的背景轨】，而此刻
     *   master.mp4 还是上一次那份、指纹也写着 done。那条刚拼好、马上要用的
     *   轨完全满足判据，一重启就没了。
     *
     * 就地删不需要任何猜测：删的就是【刚刚被这次烧录消费掉的那一条】。
     */
    try {
