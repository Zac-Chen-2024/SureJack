...
/**
 * 一条字幕最多几个字。**超过就送去做语义小拆分。**
 *
 * 【物理极限是 17，量出来的】。81 号字、左右各留 60px 安全边距 →
 ...
 *   20 字 1080px（余 0）  ← 真出界
 *
 * 【但现在取 14】（2026-08-14，用户看过 10~17 的逐档烧录对比图后定的）。
 * 17 是"塞得下"，不是"看着舒服"——铺满整个安全区在手机上依然压迫。
 * 降到 14 之后超出的部分【照样走这条语义切分】，不是机械硬断，
 * 所以代价只是多调几次 LLM：实测她那 4 条片子的超限段从 72 增到 181
 * （2.5 倍），批次从 2 变 4，每条片子多几分钱、多几十秒。
 *
 * ⚠️【必须和 project-ass.ts 的 SUBTITLE_MAX_CHARS 保持一致】——
 * 那是渲染上限，这是送 LLM 的阈值。两者不一致时，落在中间的段
 * 不会被送来切，而是在渲染时被机械硬断。
 */
export const SUBTITLE_CUT_MAX = 14
