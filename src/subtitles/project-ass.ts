 ...
 *   17  —— 配合【语义切分】：超限的行不再靠折行硬扛，而是由 LLM 按语义
 *          切成几条先后显示（见 cut-ai.ts）。17 是【量出来的】物理极限：
 *          81 号字、左右各 60px 安全边距 → 可用 960px，17 字正好 959px
 *          铺满；18 字吃掉安全边距，19 字离屏幕边只剩 5px，20 字被裁。
 *   14  —— 现在这版（2026-08-14，用户看过 10~17 的逐档烧录对比图后定的）。
 *          17 是"塞得下"的上限，不是"看着舒服"的上限——铺满整个安全区
 *          在手机上依然压迫。14 留出余白，超出的部分照样交给语义切分，
 *          所以【不是硬断，是多切一刀】。
 *
 * ⚠️【必须和 cut-ai.ts 的 SUBTITLE_CUT_MAX 保持一致】。那个是"超过多少
 * 就送 LLM 去切"的阈值，这个是"渲染时一行最多几个字"的上限。
 * 只改这一个的话，落在两数之间的段【不会送去 LLM】，会被机械硬断——
 * 那正是 14→24 那一版要消灭的"大力出奇迹 → 大力出/奇迹"。
 *
 * 【指纹侧独立，仍然是 LEGACY_SUBTITLE_MAX_CHARS】，所以历史项目
 * 一条都不会因为这几次调整被重烧。它现在的数值恰好也是 14，
 * 但那是巧合——两者用途完全不同，不要合并。
 */
