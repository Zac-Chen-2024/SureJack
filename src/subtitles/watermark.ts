...
/**
 * 每个位置【什么时候到达】：**每 1 分半一跳，六段等长**。
 *
 * 0:00 左上 → 1:30 右中 → 3:00 左下 → 4:30 右上 → 6:00 左中 → 7:30 右下
 *
 * ⚠️【是绝对时间，不随片长缩放】。用户说得很明白："如果不够长也按照这个
 * 时间分布来烧录"——四分钟的片子就只走到第三个位置，不把六段压缩进四分钟。
 *
 * ── 沿革：原来后三段是 3 分钟一跳（6/9/12 分）────────────────────────
 * 那样一来，从第 3 分钟到第 9 分钟水印【一个角都不沾】：3→6 分是"左下→右上"
 * 一整条对角线，6→9 分又从右上斜穿到左中，全程在画面中间游走。
 * 用户报"七分半的水印没有了"——其实在，只是在画面正中(541, 502)压着
 * 地铁跑酷的金色光圈，46% 不透明度，往四个角上找自然找不到。
 * 改成等长之后 7:30 正好落在右下角上。
 */
export const WATERMARK_OFFSETS_MS = [0, 90_000, 180_000, 270_000, 360_000, 450_000]

/**
 * 一轮多长。7:30 到达右下 + 1 分半（沿用同样的间隔）= **9 分**，
 * 之后回到左上重来一轮。用户说"如果更长就循环"。
 */
export const WATERMARK_CYCLE_MS = 540_000
