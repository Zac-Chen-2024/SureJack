...
describe('切句', () => {
  ...
  /*
   * ⚠️【逗号就是句末，这是这条流水线的前提】。作者的写法是"一行一句、
   * 行尾一个逗号"，全文一个句号都没有。原来不认逗号时，她 8028 字的稿子
   * 整篇被切成 1 句，分集报"正文太短"。
   */
  it('逗号就是句末', () => {
    expect(splitSentences('他站住了，回头看了一眼。')).toHaveLength(2)
  })

  it('半角逗号也算——作者行尾打的就是它', () => {
    const s = splitSentences('宫宴上，阿姐误沾迷香,\n可她早已是太子妃,')
    // 3 句不是 4：行尾逗号后面紧跟的换行并进同一句，不会多出一个空句
    expect(s).toHaveLength(3)
    expect(s.map((x) => x.text).join('')).toBe('宫宴上，阿姐误沾迷香,\n可她早已是太子妃,')
  })

  it('顿号不算句末——按它断会切出两三个字的碎句', () => {
    expect(splitSentences('他带了刀、剑、枪三样东西')).toHaveLength(1)
  })
   ...
   */
  /*
   * 换行本身也是句末（作者一行一句），所以空行不会单独成句——
   * 切出来是空白的那一段并进【上一句】。谁收留它不重要，
   * 重要的是【原文逐字保留】：拼回去必须和原文一模一样，
   * 拆主片/续集就是按 start/end 从原文上切的。
   */
  it('空行不单独成句，并进上一句', () => {
    const s = splitSentences('第一句。\n\n第二句。')
    expect(s).toHaveLength(2)
    expect(s[0]!.text).toBe('第一句。\n\n')
    expect(s[1]!.text).toBe('第二句。')
    expect(s.map((x) => x.text).join('')).toBe('第一句。\n\n第二句。')
  })
})
