...
**Files:** `web/src/components/AssetPanel.tsx`（重写）、`web/src/store/library.ts`（新）、`web/src/pages/Workspace.tsx`

```
素材
├ 背景视频
│   ┌──────────────────────────────────┐
│   │ 开头 ▏ 常规 ▏━━━ 地铁跑酷 ━━━━ │  ← 按 takeMs 等比的分段条
│   └──────────────────────────────────┘
│   3:31 开头 · 3:31 常规 · 6:01 地铁跑酷
│   共 12 段素材                              ← 点开可看用了哪些文件
├ 背景音乐
│   ○ 一笑倾城   现言 甜文
│   ● 若梦       古言 虐文        ← 选中态
...
- **没有任何上传按钮。**
- 分段条三段用同一强调色的不同明度（`accent` 全亮 / 60% / 30%），**不要引入三种新颜色**
- BGM 文件名自带标签（`一笑倾城 现言 甜文.wav`）：**首个空格前是曲名，其余是标签**。标签用小号 `ink-400` 排在曲名右侧
- 配音未生成时，分段条显示空状态：「生成配音后按时长自动排布」——因为背景长度由配音决定
- 音量平衡滑块要真能拖（见任务 #56，`bgm_volume` 后端一直在用，前端从来没接）
