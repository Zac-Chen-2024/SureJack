...
**⚠️ 导出的校验顺序变了**（Task 3 的连锁反应，第一版计划没想到）：老代码第一句是
「没有背景视频 → 400 先上传一个」，公式模式下这句话不成立——背景就是不用传的。
现在顺序是：多个上传视频 → 配音 → 素材库。**前端不能再指望「先传背景视频」这类提示**，
导出按钮的禁用理由要改成「先生成配音」。

**BGM 找不到时不阻断导出**（Task 3 的决定）：`bgmLibraryId` 指向已删除的素材时
静默不混 BGM 继续导出，而不是整条失败。成片没有背景音乐是看得见、好补救的；
导出崩了不是。前端不必为此做特殊处理。
